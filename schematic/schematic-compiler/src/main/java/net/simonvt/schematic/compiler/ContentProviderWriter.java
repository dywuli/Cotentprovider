...
import net.simonvt.schematic.annotation.NotifyQuery;
...
public class ContentProviderWriter {
    ...
    ExecutableElement defaultNotifyQuery;
    ...
    Map<String, ExecutableElement> notifyQuery = new HashMap<>();
    ...
    public ContentProviderWriter(ProcessingEnvironment processingEnv, Elements elements,
                                 Element provider, Set<? extends Element> tables) {
...
//        List<? extends Element> enclosedElements = provider.getEnclosedElements();
        ...
        for (Element enclosedElement : tables) {
            TableEndpoint tableEndpoint = enclosedElement.getAnnotation(TableEndpoint.class);
            if (!annotation.packageName().equals(tableEndpoint.packageName())) continue;
            NotifyQuery defaultNotifyQuery = enclosedElement.getAnnotation(NotifyQuery.class);
            if (defaultNotifyQuery != null) {
                this.defaultNotifyQuery = (ExecutableElement) enclosedElement;
            }
            ...
            if (tableEndpoint != null) {
                ...
                for (Element element : contentUris) {
                    ...
                    NotifyQuery notifyQuery = element.getAnnotation(NotifyQuery.class);
                    if (notifyQuery != null) {
                        String[] paths = notifyQuery.paths();
                        for (String path : paths) {
                            this.notifyQuery.put(path, (ExecutableElement) element);
                        }
                    }
    ...
    private MethodSpec getQuerySpec() {
        ...
        for (UriContract uri : uris) {
            if (uri.allowQuery) {
                ...
                if ((uri.path != null && notifyQuery.containsKey(uri.path))
                        || defaultNotifyQuery != null) {
                    CodeBlock notifyQuery = getNotifyQuery(uri);
                    spec.addCode(notifyQuery);
                } else {
                    spec.addStatement("getContext().getContentResolver().notifyChange(uri, null)");
                }
    ...
    private CodeBlock getNotifyQuery(UriContract uri) {
        ExecutableElement notifyMethod = notifyQuery.get(uri.path);
        if (notifyMethod == null) {
            notifyMethod = defaultNotifyInsert;
        }
        ...
        for (VariableElement param : parameters) {
            ...
            if (Clazz.CONTEXT.equals(ClassName.get(paramType))) {
                ...
                error(String.format("@NotifyQuery does not support parameter %s",
                        paramType.toString()));
            }
        }
